...
const assertGoogleEnv = () => {
    ...
    if (missing.length) {
        throw new Error(`Google Calendar: faltan variables de entorno: ${missing.join(', ')}`);
    }
};
...
const getCalendarClient = () => {
    try {
        ...
        try {
            const payload = (error && error.response && error.response.data) || error.errors || error.message;
            console.error('Google Calendar: error al crear evento:', payload);
        } catch (e) {
            console.error('Google Calendar: error al crear evento (no parseable):', error);
        }
...
const createEvent = async (bookingData) => {
    try {
        ...
        const endDateTimeStr = `${date}T${String(endHour).padStart(2, '0')}:${isNaN(m) ? '00' : String(m).padStart(2, '0')}:00`;

        // Mapear nombres de servicios a descripciones más detalladas
        const serviceDescriptions = {
            'corte-personalizado': 'Corte de Cabello Personalizado',
            'corte-barba-diarquia': 'Corte y Barba "La Diarquía" - Experiencia Premium',
