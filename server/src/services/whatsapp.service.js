...
const sendConfirmation = async (bookingData) => {
    try {
        ...
        if (!process.env.WHATSAPP_PHONE_NUMBER_ID || !process.env.WHATSAPP_ACCESS_TOKEN) {
            throw new Error('WhatsApp: faltan variables de entorno WHATSAPP_PHONE_NUMBER_ID o WHATSAPP_ACCESS_TOKEN');
        }
        ...
        console.log('✅ Mensaje de WhatsApp enviado exitosamente usando plantilla "confirmacion_reserva_barberia":', response.data);
        ...
        if (error.response) {
            ...
            if (status === 400) {
                throw new Error(`WhatsApp: ${data.error?.message || 'Solicitud inválida'}`);
            }

            throw new Error(`WhatsApp: Error ${status} - ${data.error?.message || 'Error desconocido'}`);
        }
