document.addEventListener('DOMContentLoaded', () => {
    const App = {
        ...
        handleScroll() {
            ...
            const { SCROLLED_CLASS, SHOW_CLASS, HEADER_SCROLL_OFFSET, BACK_TO_TOP_OFFSET } = App.config;
            const scrollY = window.pageYOffset;
        ...
        async handleContactFormSubmit(e) {
            ...
            try {
                ...
                const apiBase = (document.querySelector('meta[name="api-base"]')?.content || 'http://localhost:3000/api').trim();
                ...
                alert(`¡Gracias ${formData.name}! Tu cita ha sido recibida. Nos pondremos en contacto para confirmar.`);
                form.reset();
                if (App.DOM.dateInput) App.DOM.dateInput.type = 'text';
            } catch (error) {
                alert('Lo sentimos, ocurrió un error al agendar tu cita. Por favor, intenta más tarde.');
        ...
        async updateAvailableTimes() {
            ...
            if (!selectedDate) {
                ...
                timeInput.innerHTML = '<option value="">Primero selecciona una fecha</option>';
                return;
            }
            ...
            try {
                ...
                timeInput.innerHTML = availableHours.length ? '<option value="">Selecciona una Hora *</option>' : '<option value="">No hay horas disponibles</option>';
        ...
        animateCounter(observer) {
            ...
            const target = parseInt(badgeNumber.dataset.target || badgeNumber.textContent, 10);
            let start = 0;
            const duration = 2000;
        ...
        // --- 4. EVENT LISTENERS ---
        bindEvents() {
            const { mobileMenuToggle, navLinks, backToTopBtn, contactForm, newsletterForm, dateInput, timeInput, logo, aosElements, badgeNumber } = App.DOM;
            ...
            navLinks.addEventListener('click', (e) => {
                if (e.target.matches('a[href^="#"]')) {
                    ...
                    if (targetElement) {
                        const offsetPosition = targetElement.offsetTop - App.config.SMOOTH_SCROLL_OFFSET;
            ...
            // Formularios
            if (contactForm) contactForm.addEventListener('submit', App.handleContactFormSubmit);
            if (newsletterForm) {
            ...
            if (logo) {
                ...
                logo.addEventListener('click', () => {
                    ...
                    if (clickCount === EASTER_EGG_CLICKS) {
                        alert('🎉 ¡Has descubierto el Easter Egg de BarbeX! 🎉');
                        clickCount = 0;
                    }
                });
            }
        ...
        init() {
            App.bindEvents();
            console.log('%c🔥 BarbeX - Barbería Premium 🔥', 'font-size: 20px; color: #d4a574; font-weight: bold;');
        }
    };
